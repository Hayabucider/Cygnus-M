...
```
config/Cygnus-M.keymap   ← 唯一の正典。常にこれを基準にする。
```
...
| タイミング | ファイル名例 |
|---|---|
| 実機書き込み前 | `Cygnus-M_backup_YYYY-MM-DD_pre-flash.keymap` |
| 動作確認が取れたとき | `Cygnus-M_backup_YYYY-MM-DD_verified.keymap` |
| 大きなレイヤー変更前 | `Cygnus-M_backup_YYYY-MM-DD_pre-edit.keymap` |
...
| ファイル | 役割 |
|---|---|
| `config/Cygnus-M.keymap` | 唯一の正典。ZMK ビルドに使う実装ファイル |
...
- [ ] 実機テスト未完了のレイヤーがある場合はここに記録
- [ ] Viewer と keymap の同期状態
- [ ] 次回作業の開始点
