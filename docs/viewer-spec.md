...
```
このHTML Viewer は、config/Cygnus-M.keymap の内容を視覚的に確認するための補助ツールである。
...
| 項目 | 値 | 備考 |
|---|---|---|
| 総キー数 | 51キー | 左25 + 右25 + encoder1（右手側） |
| レイヤー数 | 11レイヤー | Layer 0〜10 |
| 左親指キー | 3キー | pos44（英数/L8）, pos45（かな/Alt）, pos46（Space/Sub） |
| 右親指キー | 4キー | pos47（→）, pos48（BSPC）, pos49（Enter/L9）, pos50（DEL） |
| 左端キー | 1キー | pos41（LCTL） |
| 右端キー | 1キー | pos51（RCTL） |
| エンコーダー | 1個 | 右手側。各レイヤーで機能が異なる（下記参照） |

> ⚠️ 上記の値は `config/Cygnus-M.keymap` の実装から抽出した確定値。
...
| 1 | Win_Sub | Windows ショートカット / Premiere Pro | pos46 `lt 1 SPACE` 長押し |
| 2 | Mac_Base | macOS ベース入力 | `to 2`（mac_switch マクロ経由） |
| 3 | Mac_Sub | macOS ショートカット / Premiere Pro | pos46 `lt 3 SPACE` 長押し |
| 4 | Android_Base | Android ベース入力（L0 と同構成） | `to 4`（and_switch マクロ経由） |
| 5 | Android_Sub | Android 向けショートカット | pos46 `lt 5 SPACE` 長押し |
| 6 | iPad_Base | iPad ベース入力（L2 と同構成） | `to 6`（pad_switch マクロ経由） |
| 7 | iPad_Sub | iPadOS 向けショートカット | pos46 `lt 7 SPACE` 長押し |
| 8 | SymNum | 記号（左手）＋テンキー（右手） | pos44 `lt 8 LANG2` 長押し |
| 9 | Nav/Func | F1〜F12 / 矢印 / マウスクリック | pos49 `lt 9 RET` 長押し または pos25 `lt 9 MINUS` 長押し |
| 10 | Management | BT設定 / OS切替 / BOOTLOADER | pos1 `lt 10 ESC` 長押し |
...
| 43 | lt 8 LANG2 | tap=英数 / hold=L8(SymNum) ← L8唯一の経路 |
...
以下は 2026-06-22〜23 に確定した内容。Viewer 修正時も変えない。
...
- **pos50（DEL）**: `lt 8 BSPC` から `kp BSPC` に変更済み → 現在は `kp DEL`
